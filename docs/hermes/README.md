...
> 基于 [NousResearch/hermes-agent](https://github.com/nousresearch/hermes-agent) v0.8.0 (2026-04-08) 的深度分析，以及将其核心优势整合到 Vitamin 的设计方案。
...
1. **Hermes 最核心的差异化** — 闭环学习系统（自主创建 Skill → 使用中自改进 → 跨会话记忆检索 → 用户建模），Vitamin 当前缺失
2. **Vitamin 已有优势** — 类型安全模块化体系、31+ Hook 拦截点、5 种 Swarm 协作模式、LLM 驱动编排器，均远超 Hermes
3. **整合策略** — 不移植 Python 代码，而是参考 Hermes 设计，在 Vitamin 的 TypeScript 模块化体系中重新实现
