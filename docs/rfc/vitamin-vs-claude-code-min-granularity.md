# Vitamin vs Claude Code 最小颗粒度对比与借鉴清单

> 范围：基于本地 `vitamin-coding` 当前工作区，以及 `aniwei/Claude-Code` 还原仓库 `b78dd22a091b717c8938ab98c736bc04825a8ee8` 的源码结构、README 和关键模块进行对比。
>
> 注意：`aniwei/Claude-Code` README 标注其为从 npm source map 还原的非官方源码，仅作为工程研究参考。本文关注架构和功能颗粒，不建议复制实现。

## 1. 总体结论

Vitamin 当前已经不是“基础版 Agent”。在 Agent loop、工具分区、延迟工具加载、多策略上下文压缩、Prompt Cache scope、权限审计、Web UI trace、多模型等方面，Vitamin 已经具备接近或超过 Claude Code 的底层能力。

真正的差距主要在产品闭环和生态层：

1. Claude Code 的 CLI/TUI、slash command、plugin、MCP、skill、remote/bridge 已形成完整用户产品。
2. Vitamin 的底层包拆分更清晰，但很多能力停留在 runtime/framework 层，缺少管理入口、信任模型、热加载、端到端验证和默认内容生态。
3. Vitamin 最值得借鉴的不是隐藏功能，而是 Claude Code 对“上下文成本、工具发现、插件生态、MCP 连接、会话恢复”的细粒度工程处理。

## 2. 架构对比

| 维度 | Claude Code | Vitamin | 判断 |
| --- | --- | --- | --- |
| 架构形态 | 单体 `src/` + feature gate | pnpm monorepo + 20+ packages | Vitamin 模块边界更清楚 |
| 运行入口 | CLI/TUI 为主，Web/remote 辅助 | CLI + service + opendev-ui | Vitamin Web 更强，CLI 弱 |
| Agent loop | Query/work loop 深度集成工具/UI/权限 | `@vitamin/agent` 状态机 + hooks | Vitamin 更可测，Claude 更产品化 |
| 配置模型 | settings + feature flags + user type | setting + model slots + profiles | Claude 实验治理更成熟 |
| 多模型 | Claude 优先 | Anthropic/OpenAI/Copilot/DeepSeek 等 | Vitamin 优势 |
| 调试 | telemetry/VCR/internal trace | devtools + audit trace + DAG UI | Vitamin 优势 |

## 3. 工具数量与实现原理

### 3.1 工具数量

| 类别 | Claude Code | Vitamin |
| --- | ---: | ---: |
...
| Agent/任务 | Agent / Task* / SendMessage | task_delegate / agent_task / agent_call / task_* |
...
| 粒度 | Claude Code | Vitamin | 借鉴点 |
| --- | --- | --- | --- |
| Tool 定义 | `buildTool()` 厚对象：schema、prompt、UI、权限、analytics | `AgentTool` 轻接口：name、description、parameters、execute | Vitamin 保持轻接口，但补 metadata 覆盖率 |
...
| 内容块 | Claude Code | Vitamin |
| --- | --- | --- |
| 身份 | Claude Code / Anthropic CLI | Vitamin / coding agent framework |
...
Vitamin 当前优势是 preset/profile/hook 链条清晰；短板是 prompt section 没有 cacheability 标记，MCP/skill instructions 注入不够一等。

### 4.4 System Prompt 内容借鉴清单

Claude Code 值得借鉴的不是完整文案，而是“内容块职责”和“注入边界”。Vitamin 应保持自身中文 lead-guidance 和 profile 体系，但把以下内容块补成一等 prompt section。

| ID | 内容块 | Claude Code 做法 | Vitamin 当前 | 借鉴方式 | 最小验收 |
| --- | --- | --- | --- | --- | --- |
...
Claude Code 的关键流程价值是：先分 section，再标记 cacheability，再并行加载动态输入，最后生成可调试的 prompt block 列表。Vitamin 可在不改变现有 `PromptManager` API 的前提下逐步升级。

| ID | 流程能力 | Claude Code 做法 | Vitamin 当前 | 借鉴方式 | 最小验收 |
| --- | --- | --- | --- | --- | --- |
...
| SP-F09 | Per-session promptRefresh 固化 | Vitamin 已有 | 已对齐 | 保留，不让运行中 preset 漂移 | fork 后继承正确 promptRefresh |
...
| 粒度 | Claude Code | Vitamin | 差距 |
| --- | --- | --- | --- |
| 连接管理 | stdio/SSE/SDK、OAuth、pending request 清理、重连 | `McpManager` + client + transport | Vitamin 骨架完整，边界处理少 |
...
| 发现 | managed/user/project/plugin/MCP/legacy commands | project/global/plugin/inline | Vitamin 少 managed/MCP/legacy |
...
| 执行 | SkillTool 带权限、进度、输出 | skill_load / skill_execute | Vitamin 工具拆分更轻 |
...
| 维度 | Claude Code | Vitamin | 差距 |
| --- | --- | --- | --- |
| 管理入口 | `/plugin` UI + CLI install/enable/disable/update | manifest 库能力 | 缺 CLI/UI |
| Manifest | commands、agents、skills、hooks、MCP、LSP、output styles、settings | tools、skills、mcpServers、permissions | Vitamin 类型少 |
...
discoverPluginManifests(roots)
  -> read plugin.json / vitamin-plugin.json
...
1. `PluginManager` 接入 `VitaminApp.start()`
...
| Ultraplan/Teleport | 依赖云端执行平台，Vitamin 暂无对应产品边界 |
| 大规模 marketplace | 需要先有 plugin trust、loader、CLI、cache |
| 完整 TUI 复刻 | Vitamin 已有 Web UI 优势，应先补 CLI 管理命令 |
...
Vitamin 最应该保留的是模块化、hook、Web trace、多模型和清晰 runtime 边界；最应该借鉴 Claude Code 的是：
...
换句话说，Vitamin 不需要复刻 Claude Code 的单体架构；需要把 Claude Code 已经打磨过的“最小行为颗粒”接到 Vitamin 现有包边界里。
