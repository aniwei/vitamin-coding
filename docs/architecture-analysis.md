...
这里要特别区分“当前源码主链”和“README / 目标态叙事”：当前源码可以直接验证到 `SettingsManager`、`ResourceManager`、`PromptManager`、`CodingSessionManager`、`ToolRegistry`、`Orchestrator` 已进入 `VitaminApp.start()` 主路径；但 `ExtensionManager`、`McpRuntime`、`bootstrap MCP` 这类说法，当前不应按“已进入默认 runtime 主链”来写。
...
1. coding 负责把 config、resources、prompt、sessions、tools、orchestrator 组合成应用容器；README 中出现的扩展 / MCP 叙事应与当前 `VitaminApp.start()` 已验证主链分开描述。
...
| Resources / 扩展 / MCP | `SettingsManager`、`ResourceManager`、`PromptManager` 已进入 `VitaminApp.start()` 主链 | README / 目录结构里提到的 ExtensionManager、McpRuntime 当前尚不能按“已进入默认启动链”描述 | 后续需要把“已实现 API”“占位目录 / 目标态”“已接入 runtime”三者明确拆开 |
...
3. 当前源码的 `start()` 主链没有显式 `bootstrap MCP` 或 `ExtensionManager` 装配步骤，因此这两类能力不应按“已验证默认主路径”来写。
...
2. 应用容器层已经成形：`coding` 把 lead、resources、settings、extensions、sessions 组织起来了。
...
| Pi-mono | extension / package 生态 | 建议融入 | P1 | `coding` 资源加载、工具扩展、包发现机制 | Vitamin 已有多包和 hooks 基础，缺的是更标准化的分发与生态接口 |
...
3. 把 Pi-mono 风格的扩展/包生态标准化，让外部能力更容易插进来。
...
如果把上一节的判断落成真正可执行的版本，建议不要按“框架来源”排任务，而要按 Vitamin 自己的架构层次排任务。更合适的拆法是三段推进：先收口默认工作流，再引入多模型分工，最后标准化扩展生态。
...
| Phase 3 | 标准化扩展与 package 生态 | `coding`、`tools`、`hooks`、CLI | 外部包、工具集、prompt preset 能稳定发现、加载、组合 |
...
2. **定义外部 package 的最小 manifest**
  让一个扩展包可以显式声明自己提供哪些 tools、prompts、hooks、skills 或 resources。
3. **把默认产品壳和扩展生态解耦**
...
| 包 | 建议改动 |
|---|---|
| `@vitamin/coding` | 扩展发现、资源装配、preset 入口 |
| `@vitamin/tools` | 工具束 manifest、命名空间和版本约束 |
| `@vitamin/hooks` | hooks preset 与扩展包的挂接约定 |
| CLI | preset 选择、扩展启停、诊断命令 |

这一阶段的完成标准应该是：

1. 外部包不改核心代码也能稳定接入一组工具和 prompt。
2. 用户可以通过 preset 快速切到不同工作模式。
3. 文档能清晰回答“如何发一个 Vitamin 扩展包”。

### 8.7 推荐实施顺序与依赖关系

这三个阶段不适合并行硬推，比较合理的依赖关系是：

1. **先做 Phase 1**，因为默认工作流不收口，多模型和扩展生态都会缺少稳定挂点。
2. **再做 Phase 2**，因为 workflow slot 需要明确的阶段边界才能落地。
3. **最后做 Phase 3**，因为扩展生态最好建立在已经稳定的工作流接口之上。
...
| 结论 | 证据位置 | 备注 |
|---|---|---|
| lead prompt 采用两阶段运行时装配 | [packages/coding/docs/lead-flow.md](../packages/coding/docs/lead-flow.md)；[packages/coding/src/app/vitamin-bootstrap.ts](../packages/coding/src/app/vitamin-bootstrap.ts)；[packages/coding/tests/lead-session.test.ts](../packages/coding/tests/lead-session.test.ts) | 说明 prompt 不是静态常量，而是 runtime artifact；测试还验证了最终 prompt 会进入 lead model 调用 |
...
| `VitaminApp.start()` 当前可直接验证的主链是 settings / resources / prompt / tools / orchestrator，而不是显式 MCP / extension bootstrap | [packages/coding/src/app/vitamin-app.ts](../packages/coding/src/app/vitamin-app.ts)；[packages/coding/src/index.ts](../packages/coding/src/index.ts) | 用于区分当前 runtime 主链与 README / 目标态叙事 |
| `reviewGate`、`retryStrategy`、`circuitBreaker`、`router` 已进入 orchestrator 装配参数 | [packages/coding/src/app/vitamin-bootstrap.ts](../packages/coding/src/app/vitamin-bootstrap.ts)；[packages/coding/src/app/types.ts](../packages/coding/src/app/types.ts) | 证明这些能力并非纯概念模块 |
