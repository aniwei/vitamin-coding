...
const ChatWrapper = () => {
  ...
  const welcome = useMemo(() => {
    ...
    if (welcomeMessage.suggestedQuestions && welcomeMessage.suggestedQuestions?.length > 0) {
      return (
        <div className={clsx('flex items-center justify-center px-4 py-12', isMobile ? 'min-h-[30vh] py-0' : 'h-[50vh]')}>
    ...
    return (
      <div className={clsx('flex min-h-[50vh] flex-col items-center justify-center gap-3 py-12', isMobile ? 'min-h-[30vh] py-0' : 'h-[50vh]')}>
        <AppIcon
          size="xl"
          iconType={appData?.site.icon_type}
          icon={appData?.site.icon}
  ...
  return (
    <Chat
      ...
      isResponding={respondingState}
      chatContainerInnerClassName={clsx('mx-auto w-full max-w-full px-4', messageList.length && 'pt-4')}
      chatFooterClassName={clsx('pb-4', !isMobile && 'rounded-b-2xl')}
      chatFooterInnerClassName={clsx('mx-auto w-full max-w-full px-4', isMobile && 'px-2')}
      onSend={doSend}
