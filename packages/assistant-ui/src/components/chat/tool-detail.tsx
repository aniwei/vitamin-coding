import { clsx } from 'clsx'
import {
  RiArrowDownSLine,
  RiArrowRightSLine,
  RiHammerFill,
  RiLoader2Line,
} from '@remixicon/react'
import { memo, useState } from 'react'
import type { ToolInfoInThought } from './types'

type ToolDetailProps = {
  payload: ToolInfoInThought
}

export const ToolDetail: React.FC<ToolDetailProps> = memo(({ payload }) => {
  const { name, label, input, isFinished, output } = payload
  const toolLabel = name.startsWith('dataset_') ? 'Knowledge' : label
  const [expand, setExpand] = useState(false)

  return (
    <div
      className={clsx(
        'rounded-xl',
        expand 
          ? 'border-[0.5px] border-components-panel-border-subtle bg-background-section-burn'
          : 'border-l-[0.25px] border-components-panel-border bg-workflow-process-bg'        
      )}
    >
      <div
        className={clsx(
          'system-xs-medium flex cursor-pointer items-center px-2.5 py-2 text-text-tertiary',
          expand && 'pb-1.5',
        )}
        onClick={() => setExpand(!expand)}
      >
        { 
          isFinished 
            ? <RiHammerFill className="mr-1 h-3.5 w-3.5" /> 
            : <RiLoader2Line className="mr-1 h-3.5 w-3.5 animate-spin" />
        }
        { isFinished ? 'Used' : 'Using' }
        <div className="mx-1 text-text-secondary">{toolLabel}</div>
        { expand ? <RiArrowRightSLine className="h-4 w-4" /> : <RiArrowDownSLine className="ml-auto h-4 w-4" /> }
      </div>
      {
        expand && <>
          <div className="mx-1 mb-0.5 radius-lg bg-components-panel-on-panel-item-bg text-text-secondary">
            <div className="system-xs-semibold-uppercase flex h-7 items-center justify-between px-2 pt-1">
              Input title
            </div>
            <div className="code-xs-regular wrap-break-word px-3 pb-2 pt-1">
              {input}
            </div>
          </div>
          <div className="mx-1 mb-1 radius-lg bg-components-panel-on-panel-item-bg text-text-secondary">
            <div className="system-xs-semibold-uppercase flex h-7 items-center justify-between px-2 pt-1">
              Output title
            </div>
            <div className="code-xs-regular wrap-break-word px-3 pb-2 pt-1">
              {output}
            </div>
          </div>
        </>
      }
    </div>
  )
})

export default ToolDetail
