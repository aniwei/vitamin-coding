...
import { Mcp } from '@/components/icons/src/vender/other'
...
const StatusIndicator: FC<StatusIndicatorProps> = ({ serverActivated }) => {
  ...
  return (
    <div className="flex items-center gap-1">
      ...
      <div className={clsx('system-xs-semibold-uppercase', serverActivated ? 'text-text-success' : 'text-text-warning')}>
        {serverActivated
          ...
          <>
            ...
            {isCurrentWorkspaceManager && (
              <Tooltip popupContent={t('overview.appInfo.regenerate', { ns: 'appOverview' }) || ''}>
                ...
                >
                  <RiLoopLeftLine className={clsx('h-4 w-4 text-text-tertiary hover:text-text-secondary', genLoading && 'animate-spin')} />
                </div>
              </Tooltip>
            )}
          </>
...
const MCPServiceCard: FC<IAppCardProps> = ({
  ...
  return (
    <>
      <div className={clsx('w-full max-w-full rounded-xl border-l-[0.5px] border-t border-effects-highlight', isMinimalState && 'h-12')}>
        <div className={clsx('relative rounded-xl bg-background-default', triggerModeDisabled && 'opacity-60')}>
          {triggerModeDisabled && (
            <TriggerModeOverlay triggerModeMessage={triggerModeMessage} />
          )}
          <div className={clsx('flex w-full flex-col items-start justify-center gap-3 self-stretch p-3', isMinimalState ? 'border-0' : 'border-b-[0.5px] border-divider-subtle')}>
