...
const FileListInLog = ({ fileList, isExpanded = false, noBorder = false, noPadding = false }: Props) => {
  ...
  return (
    <div className={clsx('px-3 py-2', expanded && 'py-3', !noBorder && 'border-t border-divider-subtle', noPadding && 'p-0!')}>
      <div className="flex justify-between gap-1">
        ...
        <div className="flex cursor-pointer items-center gap-1" onClick={() => setExpanded(!expanded)}>
          ...
          <RiArrowRightSLine className={clsx('h-4 w-4 text-text-tertiary', expanded && 'rotate-90')} />
        </div>
      </div>
      {expanded && (
