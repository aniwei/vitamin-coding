...
function create(
  node: AbstractNode,
  key: string,
  porps?: { [key: string]: any } | false,
): any {
  if (!porps) {
    return React.createElement(node.name, { 
      key, 
      ...normalizeAttrs(node.attributes) 
    }, (node.children || []).map((child, index) => create(child, `${key}-${node.name}-${index}`)))
  }
...
export const IconBase = ({
  ...
  const { data, className, onClick, style, ...rest } = props

  return create(data.icon, `svg-${data.name}`, {
    className,
    onClick,
    style,
    dataIcon: data.name,
