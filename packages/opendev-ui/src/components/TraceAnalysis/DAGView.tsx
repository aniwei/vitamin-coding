...
import { type AnyFlowNode, collapseGraph } from '../../utils/trace/collapseGraph'
...
export function DAGView({ sessionData, loading }: Props) {
  ...
  const [directSelectedData, setDirectSelectedData] = useState<
    TraceNodeData | ToolNodeData | TaskNodeData | null
  >(null)
  ...
  return (
    <div className="flex-1 flex flex-col overflow-hidden bg-bg-100">
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        {detailData && <NodeDetail data={detailData} onClose={handleCloseDetail} />}
      </div>
    </div>
  )
}
