...
export function CommandPalette({ isOpen, onClose, onOpenStatus }: CommandPaletteProps) {
  ...
  const commands: Command[] = [
    ...
    },
    {
      id: 'mode',
      label: '/mode',
      description: 'Toggle between Normal and Plan mode',
      action: () => {
        toggleMode()
        onClose()
      },
    },
    {
      id: 'status',
      label: '/status',
      description: 'Show session status dialog',
      action: () => {
        onOpenStatus()
        onClose()
      },
    },
    {
      id: 'interrupt',
      label: '/interrupt',
      description: 'Interrupt the current task',
      action: () => {
        sendInterrupt()
        onClose()
      },
    ...
    {
      id: 'autonomy',
      label: 'Cycle Autonomy',
      description: 'Cycle: Manual → Semi-Auto → Auto',
      action: () => {
        cycleAutonomy()
        onClose()
      },
    },
    {
      ...
      description: 'Cycle: Off → Low → Medium → High',
      action: () => {
        cycleThinkingLevel()
        onClose()
      },
    },
    {
      ...
      description: 'Show/hide the sessions sidebar',
      action: () => {
        toggleSidebar()
        onClose()
      },
            ...
            filtered.map((cmd, i) => (
              <button
                ...
                className={`w-full text-left px-4 py-2.5 flex items-center gap-3 text-sm transition-colors ${
                  i === selectedIndex
                    ? 'bg-accent-main-100/10 text-text-000'
