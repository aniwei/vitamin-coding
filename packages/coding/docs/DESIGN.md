# X-Mars Coding Runtime 设计

最后更新：2026-04-02  
状态：Current Runtime  
范围：@x-mars/coding、@x-mars/orchestrator、@x-mars/tools、@x-mars/prompt、@x-mars/memory

## 1. 文档目标

本文只描述当前代码中已经落地并接入运行路径的设计，不包含历史方案、对比分析或未来目标态。

`@x-mars/coding` 当前的职责是把模型、会话、工具、编排、提示词和 Hook 组装成一个可执行的 coding runtime，并提供两类能力：
...
`@x-mars/orchestrator` 当前通过 `runSession` 回调复用同一套 session runtime，支撑：
...
flowchart TD
    ...
    VA[XMarsApp.createSession] -->|agentName?| DETECT{promptPreset 自动检测}
...
`CodingSessionManager` 负责把底层 `@x-mars/session` 与 `AgentSession` 绑定起来，提供：
...
`@x-mars/prompt` 的 `PromptManager` 负责 system prompt 的加载、缓存和组装。它提供两层 API：
...
Agent profile 定义每种专用子代理的能力与配置，存放在 `@x-mars/setting` 的 `agent-profiles.json` 中。
...
它不直接创建模型或 agent，而是调用 `XMarsApp` 注入的 `runSession`：
...
`ResourceManager` 当前仍是 `XMarsApp` 的生命周期成员，但不在主动执行链路上：
...
5. `settings.model`
6. `XMarsApp.defaultModel`（构造函数传入）
...
dispatchTask(args: { prompt, subagent, category, mode, slot, ... })
  ...
  -> XMarsApp.createSession({ agentName, slot })
