...
主代理的 system prompt 由 10 个独立的 markdown section 拼接而成，每个 section 存放在 `packages/prompt/prompts/lead-guidance/` 下，全部使用中文编写。默认组装顺序为：

| 序号 | Section Key | 文件 | 职责 |
|------|-------------|------|------|
| 1 | identityAndEnvironment | identity-and-environment.md | 身份与环境 |
| 2 | safetyGuardrails | safety-guardrails.md | 安全与边界 |
| 3 | outputAndCommunication | output-and-communication.md | 输出与沟通 |
| 4 | toolUsageGuidelines | tool-usage-guidelines.md | 工具使用指引 |
| 5 | workflowOverview | workflow-overview.md | 工作流程引导 |
| 6 | phaseDiscipline | phase-discipline.md | 阶段纪律 |
| 7 | complexityRouting | complexity-routing.md | 复杂度路由 |
| 8 | reviewGuidance | review-guidance.md | 审查指引 |
| 9 | modelSlotGuidance | model-slot-guidance.md | 模型槽位指引 |
| 10 | fileStateGuidance | file-state-guidance.md | 文件状态刷新 |

section 可以通过 `AssembleOptions` 逐个开关，`PromptCache` 保证每个 section 只加载一次。

为什么保持多文件而不是合并成单文件：

1. 运行时已经依赖 section 粒度开关（`AssembleOptions`），用于不同场景裁剪 prompt。
2. Hook 注入（工具指引、环境、phase、lesson）发生在主 prompt 之后，拆分 section 更容易定位冲突来源。
3. 目前 lead-guidance 总量较小（10 个 section，约 150+ 行），维护成本低于单大文件的审阅成本。
4. 与主流框架一致：多数框架采用“多段模板 + 运行时组装”，而不是单一超长 system prompt 文件。

仅在以下条件同时满足时，才建议回退为单文件：

- section 开关不再需要；
- 运行时不再做定向注入或裁剪；
- prompt 内容趋于稳定且很少按模块迭代。
...
type PromptPresetOptions =
  | { preset?: 'main' }
  | { preset: 'subagent'; agentName: string; profile?: AgentProfile; context?: SubAgentPromptContext }
```
