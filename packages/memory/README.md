...
│                                                                     │
│  1. messages = sessionManager.buildSessionContext().messages         │
│                        ↓                                            │
...
│       )                                                             │
│                        ↓                                            │
│  4. transformedMessages = result.messages                           │
│     → 传入 Agent.prompt() 作为 LLM 上下文                          │
└─────────────────────────────────────────────────────────────────────┘
```

### Session 上下文重建与 Compaction

Session 的 `buildSessionContext()` 已经支持 CompactionEntry：

```ts
// @vitamin/session — SessionManager 的上下文重建算法
buildSessionContext():
  1. 从 leaf 沿 parentId 回溯到 root
  2. 找到最后一条 CompactionEntry
  3. 有 compaction:
     a. 注入 summary 作为首条消息
     b. 从 firstKeptEntryId 开始 emit 保留的消息
  4. 无 compaction: 按路径顺序 emit 全部消息
```

Memory 的 `MemoryManager.process()` 生成 compaction 数据后，AgentSession 调用 `sessionManager.appendCompaction()` 将其写入 session 树。下次调用 `buildSessionContext()` 时，session 会自动裁剪旧消息、注入 summary。
...
```ts
// @vitamin/coding 中的 AgentSession 改造
import { createMemoryManager, createArchiveStorage } from '@vitamin/memory'
import { createSessionStorage } from '@vitamin/session'
import type { StorageConfig } from '@vitamin/memory'
...
class AgentSession {
  ...
  constructor(options: {
    storageConfig: StorageConfig   // 统一持久化配置
    model: Model
    summarize: MemoryManagerConfig['summarize']
    // ...
  }) {
    // 同一 StorageConfig 创建两个 storage
    const sessionStorage = createSessionStorage(options.storageConfig)
    const archiveStorage = createArchiveStorage(options.storageConfig)

    this.sessionManager = SessionManager.create(cwd, sessionStorage)
    ...
    this.memoryManager = createMemoryManager({
      ...
      model: {
        ...
        maxOutput: options.model.maxOutputTokens,
      },
    })
  }
  ...
  async prompt(text: string): Promise<void> {
    ...
    // 2. Memory 处理（prune → compact → archive）
    const processed = await this.memoryManager.process(
      context.messages,
    ...
    if (processed.compacted && processed.summary) {
      this.sessionManager.appendCompaction(
        processed.summary,
        /* firstKeptEntryId */ '...',
        processed.tokensBefore,
        { archivePath: processed.archivePath },
      )
    }

    // 4. 将处理后的消息传给 Agent
    const result = await this.agent.prompt(userMessage, {
      transformContext: () => processed.messages,
    })
  }
}
```
...
class VitaminApp {
  ...
  async createSession(options?: AgentSessionOptions): Promise<AgentSession> {
    return new AgentSession({
      storageConfig: this.storageConfig,  // 传递给 session + memory
      model: options.model,
      // ...
    })
  }
