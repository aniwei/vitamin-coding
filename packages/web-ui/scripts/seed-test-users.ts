...
import { Pool } from 'pg'
import { UserTable, ChatMessageTable, ChatThreadTable } from 'lib/db/pg/schema.pg'
...
async function clearExistingTestUsers() {
  ...
  try {
    ...
    const legacyTestEmails = ['admin@testuser.com', 'editor@testuser.com', 'user@testuser.com']
    ...
    if (testUsers.length > 0) {
      ...
      for (const userId of userIds) {
        await db.delete(ChatThreadTable).where(sql`${ChatThreadTable.userId} = ${userId}`)
      }
    ...
    console.log('Note: Error during cleanup (may be expected if tables are empty):', error)
  }
}
  ...
  try {
    ...
    if (userData.role && currentUser) {
      ...
      if (currentUser.role !== userData.role) {
        ...
        console.log(`  Role already correct (${currentUser.role}) for ${userData.email}`)
      }
    }
    ...
    try {
      ...
      console.warn(`Could not retrieve existing user ${userData.email}:`, fallbackError)
    }
...
async function seedTestUsers() {
  ...
  try {
    ...
    console.log(`  Editor: ${TEST_USERS.editor.email} / ${TEST_USERS.editor.password}`)
    console.log(`  Editor2: ${TEST_USERS.editor2.email} / ${TEST_USERS.editor2.password}`)
    console.log(`  Regular: ${TEST_USERS.regular.email} / ${TEST_USERS.regular.password}`)
    ...
    console.log(`  - tests/.auth/${TEST_USERS.editor2.authFile} (editor2 user)`)
    console.log(`  - tests/.auth/${TEST_USERS.regular.authFile} (regular user)`)
  } catch (error) {
...
async function seedSampleUsageData(userIds: string[]) {
  try {
    ...
    console.log(`✅ Editor user left without usage data for empty state testing`)
  } catch (error) {
