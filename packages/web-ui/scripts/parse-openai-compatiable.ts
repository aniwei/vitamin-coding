 ...
 */
function updateEnvVariable(envFilePath: string, keyToModify: string, newValue: string): boolean {
  try {
    let envContent = ''
    ...
    console.log(`Successfully updated ${keyToModify} in ${envFilePath} to: \n\n${newValue}\n`)
    return true
  } catch (error) {
