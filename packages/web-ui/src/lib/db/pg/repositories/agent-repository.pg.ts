...
export const pgAgentRepository: AgentRepository = {
    ...
    const [result] = await db
      ...
      .where(
        and(
          ...
          eq(AgentTable.id, id),
          or(eq(AgentTable.userId, userId), eq(AgentTable.visibility, 'public')),
        ),
      )
      .returning()
  ...
  async deleteAgent(id, userId) {
    await db.delete(AgentTable).where(and(eq(AgentTable.id, id), eq(AgentTable.userId, userId)))
  },

  async selectAgents(currentUserId, filters = ['all'], limit = 50): Promise<AgentSummary[]> {
      ...
      if (filter === 'mine') {
        ...
        orConditions.push(
          and(
            ...
            or(eq(AgentTable.visibility, 'public'), eq(AgentTable.visibility, 'readonly')),
          ),
        )
