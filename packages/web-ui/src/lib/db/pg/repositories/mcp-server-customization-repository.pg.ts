...
export const pgMcpServerCustomizationRepository: McpServerCustomizationRepository = {
  async selectByUserIdAndMcpServerId({ userId, mcpServerId }) {
    const [row] = await db
      ...
      .innerJoin(McpServerTable, eq(McpServerCustomizationTable.mcpServerId, McpServerTable.id))
      .where(
        and(
  ...
  async upsertMcpServerCustomization(data) {
    ...
    const [result] = await db
      ...
      .onConflictDoUpdate({
        target: [McpServerCustomizationTable.userId, McpServerCustomizationTable.mcpServerId],
        set: {
          prompt: data.prompt ?? null,
          updatedAt: now,
        },
