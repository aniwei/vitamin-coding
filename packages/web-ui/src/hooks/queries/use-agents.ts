...
export function useAgents(options: UseAgentsOptions = {}) {
  ...
  return {
    ...
    hasAgents: (type: 'mine' | 'shared' | 'bookmarked' | 'public' | 'readonly') => {
      switch (type) {
        case 'mine':
        ...
        case 'shared':
          return agents.some((agent) => agent.userId !== currentUserId)
        case 'bookmarked':
          return agents.some((agent) => agent.userId !== currentUserId && agent.isBookmarked)
        case 'public':
...
export function useMutateAgents() {
  ...
  return (updatedAgent?: Partial<AgentSummary> & { id: string }, deleteAgent?: boolean) => {
    // Update all agent list endpoints (with or without query strings)
    mutate(
      (key) => {
        if (typeof key !== 'string') return false
        // Match /api/agent or /api/agent?... but not /api/agent/id
        return key.startsWith('/api/agent') && !key.match(/\/api\/agent\/[^/?]+/)
      },
      (cachedData: any) => {
        if (!cachedData || !Array.isArray(cachedData) || !updatedAgent) return cachedData

        // Handle agent deletion
        if (deleteAgent) {
          return cachedData.filter((agent: AgentSummary) => agent.id !== updatedAgent?.id)
        }
