...
export function ToolCallMessage({ message, hasResult }: ToolCallMessageExtProps) {
  ...
  if (message.role === 'tool_call') {
    ...
    if (typeof toolResult === 'string') {
      try {
        ...
        resultData = {
          output: toolResult,
          success:
            !toolResult.includes('::tool_error::') && !toolResult.includes('::interrupted::'),
        }
      }
    }
    ...
    if (summaryOverride) {
      summaryLines = Array.isArray(summaryOverride) ? summaryOverride : [summaryOverride]
    } else {
      try {
              ...
              return (
                <div
                  ...
                  className={`font-mono text-sm mb-1 leading-6 ${
                    isError ? 'text-danger-100' : isSuccess ? 'text-success-100' : 'text-text-300'
                  }`}
                >
                  {lineStr}
                </div>
