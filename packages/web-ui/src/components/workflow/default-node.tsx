...
export const DefaultNode = memo(function DefaultNode({ data, isConnectable, selected, id }: Props) {
  const [openNodeSelect, setOpenNodeSelect] = useState(false)

  const { fitView, getEdges, addNodes, getNode, getNodes, addEdges, updateNode } = useReactFlow()
      ...
      <ContextMenuTrigger>
        <div
          className={cn(
            ...
            data.kind !== NodeKind.Note && selected && 'border-blue-500 bg-secondary!',
            data.runtime?.status === 'fail' && 'border-destructive',
            ['success', 'running'].includes(data.runtime?.status ?? '') && 'border-green-400',
          )}
        >
          <div className='flex items-center gap-2 relative px-4'>
            {![NodeKind.Note, NodeKind.Input].includes(data.kind) && (
              <Handle
                ...
                className={cn(
                  ...
                  ['success', 'running'].includes(data.runtime?.status ?? '') && 'bg-green-400!',
                )}
            ...
            {![NodeKind.Note, NodeKind.Output, NodeKind.Condition].includes(data.kind) && (
              <Handle
                type='source'
                onConnect={() => update()}
                position={Position.Right}
                className={cn('z-10 border-none! -right-0! bg-transparent! w-4! h-4!')}
                id='right'
                isConnectable={isConnectable}
                onMouseUp={() => {
              ...
              <div className='px-4 mt-2'>
                <div className='text-xs text-muted-foreground'>
                  {data.kind === NodeKind.Note ? (
                    ...
                    <p className={cn('break-all whitespace-pre-wrap text-sm text-foreground mt-4')}>
                      {data.description}
                    </p>
                  )}
                </div>
