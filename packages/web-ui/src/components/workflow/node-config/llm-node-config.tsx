...
import { InfoIcon, MessageCirclePlusIcon, TrashIcon, VariableIcon } from 'lucide-react'
...
export const LLMNodeDataConfig = memo(function ({ data }: { data: LLMNodeData }) {
  ...
  const editable = useWorkflowStore((state) => {
    return state.processIds.length === 0 && state.hasEditAccess && !state.workflow?.isPublished
  })
    ...
    <div className='flex flex-col gap-2 text-sm h-full px-4 '>
      ...
      <div className='flex items-center flex-wrap gap-1'>
        {Object.keys(data.outputSchema.properties).flatMap((key) => {
          if (key === 'answer' && data.outputSchema.properties[key].type === 'object') {
            return Object.keys(data.outputSchema.properties[key].properties ?? {}).map(
              (property) => {
                return (
                  <div
                    key={`${key}.${property}`}
                  ...
                  >
                    ...
                    <span className='text-muted-foreground ml-2'>
                      {toAny(data.outputSchema.properties[key].properties![property])?.type}
                    </span>
                  </div>
                )
              },
      ...
      <div className='flex flex-col gap-2'>
        ...
        <Button
          ...
          onClick={addMessage}
        >
          <MessageCirclePlusIcon className='size-4' /> {t('Workflow.addMessage')}
        </Button>
      </div>
...
export const LLMNodeDataStack = memo(function ({ data }: { data: LLMNodeData }) {
  ...
  return (
    <div className='flex flex-col gap-1 px-4 mt-4'>
      <div className='border bg-input text-[10px] rounded px-2 py-1 flex items-center gap-1'>
        ...
        <span className='text-xs text-muted-foreground '>{isTextResponse ? 'text' : 'object'}</span>
      </div>
    </div>
  )
})
