...
import { InputNodeData, WorkflowNodeData } from 'lib/ai/workflow/workflow.interface'
import { memo, useCallback } from 'react'
import { Feild, EditJsonSchemaFieldPopup, getFieldKey } from '../../edit-json-schema-field-popup'
      ...
      updateNodeData(data.id, (node) => {
        ...
        const outputSchema = {
          ...
          required: !field.required
            ...
            : Array.from(new Set([...(prev.outputSchema.required ?? []), field.key])),
        }
        return {
          outputSchema,
        }
  ...
  const removeField = useCallback(
    (key: string) => {
      updateNodeData(data.id, (node) => {
        ...
        const outputSchema = {
          ...prev.outputSchema,
          properties: objectFlow(prev.outputSchema.properties).filter((_, k) => k != key),
        ...
        {Object.entries(data.outputSchema.properties ?? {}).map(([key, value]) => (
          ...
          >
            ...
            <span className='block group-hover/item:hidden text-xs text-muted-foreground'>
              <span className='text-[10px] text-destructive'>{checkRequired(key) ? '*' : ' '}</span>
              {getFieldKey(value)}
            </span>
...
export const OutputSchemaStack = memo(function ({ data }: { data: WorkflowNodeData }) {
