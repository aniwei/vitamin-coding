...
export function VariableMentionItem({
  ...
  return (
    ...
    >
      ...
      <span
        className={cn(notFound ? 'text-destructive' : 'text-blue-500', 'min-w-0 truncate flex-1')}
      >
        {path.join('.')}
      </span>
      {onRemove ? (
        <XIcon className='text-muted-foreground size-2.5 cursor-pointer' onClick={onRemove} />
      ) : null}
    </div>
  )
}
