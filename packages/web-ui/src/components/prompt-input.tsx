...
export default function PromptInput({
  ...
  const [globalModel, threadMentions, threadFiles, threadImageToolModel, appStoreMutate] = appStore(
    useShallow((state) => [
      state.chatModel,
  ...
  const canUploadImages = supportedFileMimeTypes?.some((mime) => mime.startsWith('image/')) ?? true
  ...
  const submit = () => {
    ...
    setInput('')
    const attachmentParts = uploadedFiles.reduce<Array<FileUIPart | TextUIPart | any>>(
      (acc, file) => {
        const isFileSupported = isFilePartSupported(file.mimeType, supportedFileMimeTypes)
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      if (e.key === 'Escape' && threadId && (mentions.length > 0 || imageToolModel)) {
        e.preventDefault()
        e.stopPropagation()
        appStoreMutate(() => ({
          threadMentions: {},
          ...
          <div className='shadow-lg overflow-hidden rounded-4xl backdrop-blur-sm transition-all duration-200 bg-muted/60 relative flex w-full flex-col cursor-text z-10 items-stretch focus-within:bg-muted hover:bg-muted focus-within:ring-muted hover:ring-muted'>
                ...
                {mentions.map((mention, i) => {
                  return (
                    <div key={i} className='flex items-center gap-2'>
                      {mention.type === 'workflow' || mention.type === 'agent' ? (
                        <Avatar
                          className='size-6 p-1 ring ring-border rounded-full flex-shrink-0'
                          style={mention.icon?.style}
                        >
                          <AvatarImage
                            src={mention.icon?.value || EMOJI_DATA[i % EMOJI_DATA.length]}
                          />
                      ...
                      <div className='flex flex-col flex-1 min-w-0'>
                        <span className='text-sm font-semibold truncate'>{mention.name}</span>
                        {mention.description ? (
            ...
            <div className='flex flex-col gap-3.5 px-5 pt-2 pb-4'>
              ...
              <div className='flex w-full items-center z-30'>
                ...
                <DropdownMenu open={isUploadDropdownOpen} onOpenChange={setIsUploadDropdownOpen}>
                  ...
                  <DropdownMenuContent align='start' side='top'>
                    <DropdownMenuItem
                      ...
                      disabled={modelInfo?.isImageInputUnsupported || !canUploadImages}
                ...
                {!isLoading && !input.length && !voiceDisabled ? (
                  ...
                  >
                    {isLoading ? (
                      <Square size={16} className='fill-muted-foreground text-muted-foreground' />
                    ) : (
              ...
              {uploadedFiles.length > 0 && (
                <div className='flex flex-wrap gap-2'>
                  {uploadedFiles.map((file) => {
                    ...
                    const imageSrc = file.previewUrl || file.url || file.dataUrl || ''
                    ...
                    return (
                      ...
                      >
                        {isImage ? (
                          ...
                          <img src={imageSrc} alt={file.name} className='w-24 h-24 object-cover' />
                        ) : (
                                    ...
                                    onClick={async () => {
                                      try {
                                        ...
                                        if (!res.ok) {
                                          ...
                                          toast.error(e.error || 'Failed to ingest file')
                                          return
                                        }
