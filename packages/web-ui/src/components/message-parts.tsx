...
export const UserMessagePart = memo(
  function UserMessagePart({
    ...
    const displayText =
      expanded || !isLongText ? part.text : truncateString(part.text, MAX_TEXT_LENGTH)
    ...
    return (
      <div className='flex flex-col gap-2 items-end my-2'>
        ...
        >
          ...
          <p className={cn('whitespace-pre-wrap text-sm break-words')}>{displayText}</p>
          {isLongText && (
            <Button
              variant='ghost'
              size='sm'
...
export const AssistMessagePart = memo(function AssistMessagePart({
  ...
  const handleModelChange = (model: ChatModel) => {
    ...
    safe(() => setIsLoading(true))
      .ifOk(() =>
        threadId ? deleteMessagesByChatIdAfterTimestampAction(message.id) : Promise.resolve(),
      )
      .ifOk(() =>
        setMessages((messages) => {
  ...
  return (
    <div className={cn(isLoading && 'animate-pulse', 'flex flex-col gap-2 group/message')}>
      <div
        data-testid='message-content'
          ...
          {!readonly && (
            <>
              ...
              <Tooltip>
                ...
                <TooltipContent className='text-destructive'>Delete Message</TooltipContent>
              </Tooltip>
            </>
          )}
                      ...
                      <div className='flex flex-col gap-2'>
                        <h4 className='text-sm font-semibold text-foreground flex items-center gap-2'>
                          ...
                          <span className='text-xs text-muted-foreground font-normal'>
                            {message.parts.filter((v) => v.type != 'step-start').length} Steps
                          </span>
                        </h4>
  ...
  return (
    ...
    >
      <div className='flex flex-row gap-2 items-center text-ring hover:text-primary transition-colors'>
        ...
        <button data-testid='message-reasoning-toggle' type='button' className='cursor-pointer'>
      ...
      <div className='pl-4'>
        <AnimatePresence initial={false}>
          {isExpanded && (
            ...
            >
              <Markdown>{reasoningText || (isThinking ? '' : "Hmm, let's see...🤔")}</Markdown>
            </motion.div>
          )}
        </AnimatePresence>
      </div>
...
export const ToolMessagePart = memo(
  ({
    ...
    const isExecuting = useMemo(() => {
      if (isWorkflowTool)
        return (result as VercelAIWorkflowToolStreamingResult)?.status == 'running'
              ...
              <div className='p-1.5 text-primary bg-input/40 rounded'>
                {isExecuting ? (
                  ...
                  <Avatar className='size-3.5'>
                    <AvatarImage
                      src={(result as VercelAIWorkflowToolStreamingResult).workflowIcon?.value}
                    />
            ...
            <div className='flex gap-2 py-2'>
              ...
              <div className='w-full flex flex-col gap-2'>
                ...
                {!result ? null : isWorkflowTool ? (
                  <WorkflowInvocation result={result as VercelAIWorkflowToolStreamingResult} />
                ) : (
                  <div
                    className={cn(
  ...
  (prev, next) => {
    ...
    if (prev.isManualToolInvocation !== next.isManualToolInvocation) return false
...
export const FileMessagePart = memo(({ part, isUserMessage }: FileMessagePartProps) => {
  ...
  const secondaryLabel =
    part.mediaType && part.mediaType !== 'application/octet-stream' ? part.mediaType : undefined
  ...
  if (isImage && fileUrl) {
    return (
      ...
      >
        ...
        <img src={fileUrl} alt={part.filename || 'Uploaded image'} className='w-full h-auto' />
        {part.filename && (
          <div className='px-3 py-2 bg-muted text-sm text-muted-foreground'>{part.filename}</div>
        )}
      </div>
    )
  }
        ...
        <div className='flex-1 min-w-0 space-y-1 pr-3'>
          ...
          <div
            className={cn(
              ...
              isUserMessage ? 'text-accent-foreground/70' : 'text-muted-foreground',
            )}
          >
            <Badge
              variant='outline'
              className={cn(
                ...
                isUserMessage && 'border-accent-foreground/30 text-accent-foreground/90',
              )}
            >
              {fileExtension}
            </Badge>
