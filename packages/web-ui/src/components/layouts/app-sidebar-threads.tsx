...
export function AppSidebarThreads() {
  ...
  const displayThreadList = useMemo(() => {
    ...
    return !isExpanded && hasExcessThreads ? threadList.slice(0, MAX_THREADS_COUNT) : threadList
  ...
  const threadGroupByDate = useMemo(() => {
    ...
    displayThreadList.forEach((thread) => {
      const threadDate =
        (thread.lastMessageAt ? new Date(thread.lastMessageAt) : new Date(thread.createdAt)) ||
        new Date()
          ...
          <SidebarMenu>
            <SidebarMenuItem>
              <SidebarGroupLabel className=''>
                <h4 className='text-xs text-muted-foreground'>{t('recentChats')}</h4>
              </SidebarGroupLabel>
                    ...
                    {isFirst && (
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent side='right' align='start'>
                          <DropdownMenuItem variant='destructive' onClick={handleDeleteAllThreads}>
                            <Trash />
                            {t('deleteAllChats')}
                          </DropdownMenuItem>
                          <DropdownMenuItem
