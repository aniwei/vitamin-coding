...
import { ArrowUpRightIcon, ChevronDown, ChevronUp, MoreHorizontal, PlusIcon } from 'lucide-react'
      ...
      if (currentThreadId) {
        appStore.setState((prev) => {
          ...
          const target = currentMentions.find(
            (mention) => mention.type == 'agent' && mention.agentId === agent.id,
          )
          ...
          return {
            threadMentions: {
              ...prev.threadMentions,
              [currentThreadId]: [...currentMentions.filter((v) => v.type != 'agent'), newMention],
            },
          }
        })
      } else {
          ...
          {isLoading ? (
            ...
            <div className='flex flex-col'>
              <div className='relative'>
                ...
                <div className={cn('w-full', expanded && 'max-h-[400px] overflow-y-auto')}>
                  {(expanded ? agents : agents.slice(0, DISPLAY_LIMIT))?.map((agent, i) => {
                    return (
                      <SidebarMenu key={agent.id} className='group/agent mr-0 w-full'>
                        <SidebarMenuItem
                          className='px-2 cursor-pointer w-full'
                          onClick={() => handleAgentClick(agent.id)}
                        >
                          <SidebarMenuButton asChild className='data-[state=open]:bg-input! w-full'>
                            <div className='flex gap-1 w-full min-w-0'>
                              ...
                              <div className='flex items-center min-w-0 w-full'>
                                <p className='truncate' data-testid='sidebar-agent-name'>
                                  {agent.name}
                                </p>
                              </div>
                              <div
                              ...
                              >
                                <AgentDropdown agent={agent} side='right' align='start'>
