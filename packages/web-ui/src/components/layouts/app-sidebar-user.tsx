      ...
      <SidebarMenuItem>
        <DropdownMenu>
          ...
          >
            ...
            <DropdownMenuItem
              onClick={() => {
                window.open('https://github.com/cgoinglove/better-chatbot/issues/new', '_blank')
              }}
            >
        ...
        <DropdownMenuSubContent className='w-48'>
          <DropdownMenuLabel className='text-muted-foreground w-full flex items-center'>
            ...
            >
              <div
                className={cn(
                  theme === 'dark' && 'bg-accent ring ring-muted-foreground/40 text-foreground',
                  'p-1 rounded-full',
                )}
              >
              ...
              <div
                className={cn(
                  theme === 'light' && 'bg-accent ring ring-muted-foreground/40 text-foreground',
                  'p-1 rounded-full',
                )}
              >
    ...
    <DropdownMenuSub>
      ...
      <DropdownMenuPortal>
        <DropdownMenuSubContent className='w-48 max-h-96 overflow-y-auto'>
          <DropdownMenuLabel className='text-muted-foreground'>{t('language')}</DropdownMenuLabel>
          {SUPPORTED_LOCALES.map((locale) => (
            <DropdownMenuCheckboxItem
              key={locale.code}
              checked={locale.code === currentLocale}
              onCheckedChange={() => locale.code !== currentLocale && handleOnChange(locale.code)}
            >
              {locale.name}
            </DropdownMenuCheckboxItem>
          ))}
