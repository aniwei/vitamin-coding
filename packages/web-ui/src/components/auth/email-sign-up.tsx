...
export default function EmailSignUp({ isFirstUser }: { isFirstUser: boolean }) {
  ...
  const successEmailStep = async () => {
    ...
    const exists = await safeProcessWithLoading(() => existsByEmailAction(formData.email)).orElse(
      false,
    )
    if (exists) {
    ...
    <Card className='w-full md:max-w-md bg-background border-none mx-auto gap-0 shadow-none animate-in fade-in duration-1000'>
      ...
      <CardContent>
        <div className='flex flex-col gap-2'>
              ...
              {formData.password && (
                <div className='space-y-1 mt-2'>
                  <div className='flex items-center gap-2 text-xs'>
                    ...
                    <span
                      className={
                        passwordValidation.hasMinLength ? 'text-primary' : 'text-muted-foreground'
                      }
                    >
                      8-20 characters
                    </span>
                  ...
                  <div className='flex items-center gap-2 text-xs'>
                    ...
                    <span
                      className={
                        passwordValidation.hasLetter ? 'text-primary' : 'text-muted-foreground'
                      }
                    >
                      At least one letter
                    </span>
                  ...
                  <div className='flex items-center gap-2 text-xs'>
                    ...
                    <span
                      className={
                        passwordValidation.hasNumber ? 'text-primary' : 'text-muted-foreground'
                      }
                    >
                      At least one number
                    </span>
          ...
          <p className='text-muted-foreground text-xs mb-6'>{steps[step - 1]}</p>
