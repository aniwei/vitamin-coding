...
export function AgentsList({
  ...
  const [deletingAgentLoading, setDeletingAgentLoading] = useState<string | null>(null)
  const [visibilityChangeLoading, setVisibilityChangeLoading] = useState<string | null>(null)

  const { data: allAgents } = useSWR('/api/agent?filters=mine,shared', fetcher, {
    fallbackData: [...initialMyAgents, ...initialSharedAgents],
  })

  const myAgents =
    allAgents?.filter((agent: AgentSummary) => agent.userId === userId) || initialMyAgents

  const sharedAgents =
    allAgents?.filter((agent: AgentSummary) => agent.userId !== userId) || initialSharedAgents

  const { toggleBookmark: toggleBookmarkHook, isLoading: isBookmarkLoading } = useBookmark({
    itemType: 'agent',
  })
          ...
          {sharedAgents.length === 0 && (
            <Card className='col-span-full bg-transparent border-none'>
              <CardHeader className='text-center py-12'>
                <CardTitle>
                  {canCreate ? t('Agent.noSharedAgents') : t('Agent.noAvailableAgents')}
                </CardTitle>
                <CardDescription>
                  {canCreate
