    ...
    return mentions.map((m, i) => (
      <div
        ...
        className={cn(
          ...
          hasEditAccess && 'hover:ring hover:ring-destructive group cursor-pointer',
        )}
        onClick={(e) => {
          e.stopPropagation()
          if (hasEditAccess) {
      ...
      >
        <div className='p-0.5'>
          {m.type === 'defaultTool' ? (
            <DefaultToolIcon name={m.name as DefaultToolName} className='size-3' />
          ) : m.type === 'mcpServer' ? (
            <MCPIcon className='size-3' />
          ) : m.type === 'workflow' ? (
            <Avatar style={m.icon?.style} className='size-3 ring-[1px] ring-input rounded-full'>
    ...
    >
      ...
      >
        <div className='flex gap-2 items-center flex-wrap mr-auto'>
          {isLoading ? (
            <span className='text-sm text-muted-foreground'>{t('Agent.loadingTools')}</span>
          ) : selectedMentions.length === 0 ? (
            <span className='text-sm text-muted-foreground'>{t('Agent.addTools')}</span>
          ) : (
            selectedMentions
          )}
        </div>
