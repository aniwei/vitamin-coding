...
import { RandomDataGeneratorExample, WeatherExample } from 'lib/ai/agent/example'
import { notify } from 'lib/notify'

const defaultConfig = (): PartialBy<Omit<Agent, 'createdAt' | 'updatedAt' | 'userId'>, 'id'> => {
  return {
    name: '',
    description: '',
    icon: {
...
export default function EditAgent({
  ...
  const isBookmarkToggleLoading = useMemo(
    () => (initialAgent?.id && isBookmarkToggleLoadingFn(initialAgent?.id)) || false,
    [initialAgent?.id, isBookmarkToggleLoadingFn],
  )

  const { data: mcpList, isLoading: isMcpLoading } = useMcpList()
  const { data: workflowToolList, isLoading: isWorkflowLoading } = useWorkflowToolList()
  ...
  const handleBookmarkToggle = useCallback(async () => {
    ...
    safe(async () => {
      ...
      .ifFail(handleErrorWithToast)
  }, [initialAgent?.id, toggleBookmark, agent.isBookmarked, isBookmarkToggleLoading])
  ...
  const isLoading = useMemo(() => {
    return isLoadingTool || isSaving || isVisibilityChangeLoading || isBookmarkToggleLoading
  }, [isLoadingTool, isSaving, isVisibilityChangeLoading, isBookmarkToggleLoading])
      ...
      <div className='z-10 relative flex flex-col gap-4 px-8 pt-8 pb-14 max-w-3xl h-full mx-auto'>
              ...
              <>
                ...
                <DropdownMenu>
                  ...
                  <DropdownMenuContent className='w-54' align='end'>
                    <DropdownMenuItem onClick={() => setAgent(RandomDataGeneratorExample)}>
                      <div className='flex items-center gap-2'>
                        <span>🎲</span>
        ...
        <div className='flex gap-4 mt-4'>
          <div className='flex flex-col justify-between gap-2 flex-1'>
            <Label htmlFor='agent-name'>{t('Agent.agentNameAndIconLabel')}</Label>
            {false ? (
        ...
        <div className='flex flex-col gap-2'>
          <Label htmlFor='agent-description'>{t('Agent.agentDescriptionLabel')}</Label>
          {false ? (
            <Skeleton className='w-full h-10' />
          ) : (
            <Input
        ...
        <div className='mt-10 flex items-center gap-2'>
          <p className='text-sm text-muted-foreground'>{t('Agent.agentSettingsDescription')}</p>
        </div>
