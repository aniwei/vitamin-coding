...
export function UserInfoCard({
  ...
  const [, detailsUpdateFormAction, isPending] = useActionState<UpdateUserActionState, FormData>(
  ...
  const renderInlineEditField = (
    ...
    return (
      <Tooltip>
        ...
        {canEdit && (
          <TooltipContent>
            {disabled ? t('emailCannotBeModifiedSSO') : t('clickToEdit', { field: fieldType })}
          </TooltipContent>
        )}
      </Tooltip>
    )
      ...
      <CardContent className='space-y-6 pt-6'>
        ...
        <div className='flex items-start gap-4'>
          ...
          <div className='flex-1 space-y-3'>
            ...
            <div className='flex items-center gap-2'>
              {renderInlineEditField('name', user.name, <User className='h-4 w-4 text-blue-600' />)}
            ...
            {!!userAccountInfo?.oauthProviders?.length && (
              <div className='flex items-center gap-2 px-2 -mx-2'>
                ...
                <p className='text-xs text-muted-foreground'>{t('emailCannotBeModifiedOAuth')}</p>
              </div>
            )}
          </div>
        </div>
        ...
        <div className='grid gap-4 sm:grid-cols-2 pt-4 border-t'>
          <div className='space-y-2'>
            <Label className='text-sm font-medium text-muted-foreground'>{tCommon('joined')}</Label>
