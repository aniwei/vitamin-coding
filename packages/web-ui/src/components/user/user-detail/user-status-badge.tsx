...
export function UserStatusBadge({
  ...
  const [_, banStatusAction, isPending] = useActionState<UpdateUserBanStatusActionState, FormData>(
  ...
  const renderBadge = () => {
    const badgeElement = user.banned ? (
      <Badge
        ...
        className={cn(
          canModify && 'cursor-pointer hover:bg-destructive/80 transition-colors',
  ...
  return (
    <>
      <AlertDialog open={showConfirmDialog} onOpenChange={setShowConfirmDialog}>
        {canModify ? <AlertDialogTrigger>{renderBadge()}</AlertDialogTrigger> : renderBadge()}
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>{willBan ? t('banUser') : t('unbanUser')}</AlertDialogTitle>
            <AlertDialogDescription>
              {willBan
          ...
          <form action={banStatusAction}>
            ...
            <div className='flex justify-end gap-2'>
              ...
              <AlertDialogAction
                disabled={isPending}
                className={willBan ? 'bg-destructive hover:bg-destructive/90' : ''}
                asChild
              >
                <SubmitButton className={willBan ? 'bg-destructive hover:bg-destructive/90' : ''}>
                  {isPending
                    ? willBan
                      ? t('banning')
                      : t('unbanning')
