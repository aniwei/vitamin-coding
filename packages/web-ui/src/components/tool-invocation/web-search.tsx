...
import { AlertTriangleIcon, CheckIcon, LoaderIcon, SearchIcon } from 'lucide-react'
...
function areEqual(
  ...
  if (prevPart.state.startsWith('output') && !equal(prevPart.output, toAny(nextPart).output))
    return false
  return true
}
...
export function WebSearchGroupPart({ parts }: { parts: ToolUIPart[] }) {
  ...
  const allCompleted = useMemo(() => parts.every((p) => p.state.startsWith('output')), [parts])
  ...
  const flatResults = useMemo(() => {
    return partStatuses.filter((s) => s.result?.results).flatMap((s) => s.result!.results)
  }, [partStatuses])
