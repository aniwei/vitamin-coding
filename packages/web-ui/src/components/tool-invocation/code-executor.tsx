...
import { CodeRunnerResult, LogEntry } from 'lib/code-runner/code-runner.interface'
        ...
        >
          ...
          <div className='flex-1 min-w-0 whitespace-pre-wrap gap-1'>
            {log.args.map((arg, i) => {
              ...
              return (
                <span key={i}>
                  {isString(arg?.value) ? arg.value.toString() : JSON.stringify(arg.value ?? arg)}
                </span>
              )
            })}
          </div>
  ...
  const logContainer = useMemo(() => {
    ...
    return (
      <div className='p-4 text-[10px] text-foreground flex flex-col gap-1 border-t'>
        ...
        {logs}
        {isRunning && <div className='ml-3 animate-caret-blink text-muted-foreground'>|</div>}
      </div>
    )
  }, [logs, isRunning])

  useEffect(() => {
    if (onResult && part.input && part.state == 'input-available' && !isRun.current) {
      isRun.current = true
