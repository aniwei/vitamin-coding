  ...
  const renderCollapsible = (isArray: boolean) => {
    ...
    return (
      <div className='flex flex-col'>
        ...
        >
          {name && (
            <span className='mr-1 text-muted-foreground  hover:text-foreground'>{name}:</span>
          )}
          {isExpanded ? (
  ...
  const renderString = (value: string) => {
    ...
    return (
      <div className='flex mr-1 rounded group hover:bg-input/30'>
        {name && <span className='mr-1 text-muted-foreground  hover:text-foreground'>{name}:</span>}
        <pre
          className={clsx(typeStyleMap.string, 'cursor-pointer group-hover:text-green-500')}
          onClick={() => setIsExpanded(!isExpanded)}
