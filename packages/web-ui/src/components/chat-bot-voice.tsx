...
export function ChatBotVoice() {
  ...
  const [agentId, appStoreMutate, voiceChat, model, allowedMcpServers, mcpList] = appStore(
    useShallow((state) => [
  ...
  const toolMentions = useMemo<ChatMention[]>(() => {
    if (!agentId) {
      ...
      return mcpList
        .filter((v) => {
          return v.id in allowedMcpServers && allowedMcpServers[v.id]?.tools?.length
        })
        .flatMap((v) => {
    ...
    return agent?.instructions.mentions?.filter((v) => v.type === 'mcpTool') ?? []
  ...
  const mcpTools = useMemo<EnabledTools[]>(() => {
    const mcpMentions = toolMentions.filter((v) => v.type === 'mcpTool') as Extract<
      ChatMention,
      { type: 'mcpTool' }
    >[]
      ...
      <div className='max-w-4xl mx-auto flex flex-col px-6 gap-6 pb-44 min-h-0 min-w-0'>
        {messages.map((message) => (
          ...
          >
            {!message.completed ? (
              <MessageLoading
                className={cn(message.role == 'user' ? 'text-muted-foreground' : 'text-foreground')}
              />
            ) : (
                  ...
                  return (
                    <p key={index}>
                      {(part.text || '...')
                        ...
                        .split(' ')
                        .map((word, wordIndex) => (
                          <span key={wordIndex} className='animate-in fade-in duration-3000'>
                            {word}{' '}
                          </span>
                        ))}
                    </p>
          ...
          <div className='animate-in fade-in-50 duration-1000'>
            <p className='text-2xl md:text-3xl lg:text-4xl font-semibold leading-tight tracking-wide'>
              {textPart.text?.split(' ').map((word, wordIndex) => (
                <span key={wordIndex} className='animate-in fade-in duration-5000'>
                  {word}{' '}
                </span>
              ))}
            </p>
