...
export default async function AgentsPage() {
  ...
  const allAgents = await agentRepository.selectAgents(session.user.id, ['mine', 'shared'], 50)

  // Separate into my agents and shared agents
  const myAgents = allAgents.filter((agent) => agent.userId === session.user.id)
  const sharedAgents = allAgents.filter((agent) => agent.userId !== session.user.id)
