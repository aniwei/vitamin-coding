...
export async function PUT(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!canEdit) {
    return Response.json({ error: 'Only editors and admins can edit agents' }, { status: 403 })
  }
...
export async function DELETE(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!canDelete) {
    return Response.json({ error: 'Only editors and admins can delete agents' }, { status: 403 })
  }
