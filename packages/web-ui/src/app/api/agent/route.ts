...
export async function GET(request: Request) {
  ...
  try {
    ...
    const agents = await agentRepository.selectAgents(session.user.id, filters, limit)
...
export async function POST(request: Request): Promise<Response> {
  ...
  if (!hasPermission) {
    return Response.json({ error: "You don't have permission to create agents" }, { status: 403 })
  }
