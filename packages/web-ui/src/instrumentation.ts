...
export async function register() {
  if (process.env.NEXT_RUNTIME === 'nodejs') {
    ...
    if (!IS_VERCEL_ENV) {
      ...
      const runMigrate = await import('./lib/db/pg/migrate.pg').then((m) => m.runMigrate)
      ...
      const initMCPManager = await import('./lib/ai/mcp/mcp-manager').then((m) => m.initMCPManager)
