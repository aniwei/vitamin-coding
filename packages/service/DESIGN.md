...
## 模块分层
- `src/coding-service.ts`：coding-service.ts 模块实现。
- `src/create-app.ts`：create-app.ts 模块实现。
- `src/debug-bridge.ts`：debug-bridge.ts 模块实现。
- `src/event-bridge.ts`：event-bridge.ts 模块实现。
- `src/index.ts`：index.ts 模块实现。
- `src/routes/`：routes 相关实现。
- `src/types.ts`：types.ts 模块实现。
- `src/websocket-manager.ts`：websocket-manager.ts 模块实现。
