...
const logger = createLogger('@vitamin/tools:mcp-adapter')

// 将 MCP JSON Schema → Zod Schema
function jsonSchemaPropertyToZod(prop: McpJsonSchemaProperty): z.ZodType {
  switch (prop.type) {
    case 'string':
      if (prop.enum) {
...
function mcpSchemaToZod(tool: McpToolDefinition): z.ZodType {
...
function mcpContentToToolContent(content: McpContent[]): ToolResult['content'] {
...
export function createMcpToolAdapter(
  ...
  // 工具名加上 server 前缀避免冲突: mcp__{serverName}__{toolName}
